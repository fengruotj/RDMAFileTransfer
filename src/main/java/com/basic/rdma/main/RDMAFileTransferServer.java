...
public class RDMAFileTransferServer implements RdmaEndpointFactory<RDMAFileTransferServer.CustomServerEndpoint> {
    ...
    public void run() throws Exception {
        ...
        File file = new File("/home/zhanyang/test.log");
        FileInputStream in = new FileInputStream(file);
        long fileLen = file.length();
        byte[] buf = new byte[100];
        double cnt = Math.ceil((double) fileLen / buf.length);
        System.out.println("the send times : " + cnt);
        ...
        for (int i = 0; i <cnt ; i++) {
            ...
            System.err.println("cnts="+i);
            int read = in.read(buf);
            String str = new String(buf, 0, read);
            dataBuf.asCharBuffer().put(str);
            //dataBuf.put(buf,0,read);
            dataBuf.clear();
